...
class TextInput(object):
    ...
    def __init__(self, background, screen, text, x, y):
        ...
        self.cursorlayer = pygame.Surface((3,35))
        self.screen = screen
        self.cursorvis = True
    ...
    def backspace(self):
        ...
        if self.cursorpos == len(self.move): return
...
class VirtualKey(object):
    ...
    def draw(self, screen, background, shifted=False, forcedraw=False):
        ...
        color = (255,255,255)
        if self.bskey:
            pygame.draw.line(templayer, color, (38,25), (10,25),2)
            pygame.draw.line(templayer, color, (10,25), (15,20),2)
            pygame.draw.line(templayer, color, (10,26), (15,31),2)
        elif self.enter:
            pygame.draw.line(templayer, color, (65,15), (75,15),2)
            pygame.draw.line(templayer, color, (75,15), (75,31),2)
            pygame.draw.line(templayer, color, (75,31), (25,31),2)
...
class VirtualKeyboard(object):
    ...
    def run(self, screen, text=''):
        ...
        self.keylayer = pygame.Surface((1024,640))
        self.keylayer.fill((0, 0, 0))
        self.keylayer.set_alpha(100)
        self.screen.blit(self.keylayer,(0,0))

        self.keys = []
        self.textbox = pygame.Surface((660,30))
        self.text = text
        self.caps = False
        ...
        self.input = TextInput(self.background,self.screen,self.text,660,230)
            ...
            if events <> None:
                for e in events:
                    if (e.type == KEYDOWN):
                        if e.key == K_ESCAPE:
                            self.clear()
                            return "" # Return what we started with
                        if e.key == K_RETURN:
                            self.clear()
                            return self.input.text[len(text):] # Return what the user entered
                        if e.key == K_LEFT:
    ...
    def addkeys(self):
        ...
        onekey = VirtualKey('ENTER',x,y,102)
        onekey.font = self.font
        onekey.enter = True
        self.keys.append(onekey)
